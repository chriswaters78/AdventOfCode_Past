            ...
            for (long p3 = 5; p3 < 10; p3++)
            {
                for (long p4 = 5; p4 < 10; p4++)
                {
                    for (long p5 = 5; p5 < 10; p5++)
                    {
                        ...
                        var amp1 = new Computer("AMP1", input.ToArray(), lazyConcat(new[] { p1, 0 }, new Lazy<IEnumerable<long>>(() => amp5))).Run();
                        var amp2 = new Computer("AMP2", input.ToArray(), lazyConcat(new[] { p2 }, new Lazy<IEnumerable<long>>(() => amp1))).Run();
                        var amp3 = new Computer("AMP3", input.ToArray(), lazyConcat(new[] { p3 }, new Lazy<IEnumerable<long>>(() => amp2))).Run();
                        var amp4 = new Computer("AMP4", input.ToArray(), lazyConcat(new[] { p4 }, new Lazy<IEnumerable<long>>(() => amp3))).Run();
                        amp5 = new Computer("AMP5", input.ToArray(), lazyConcat(new[] { p5 }, new Lazy<IEnumerable<long>>(() => amp4))).Run();

                        output = Math.Max(output, amp5.Last());
                    }
                }
            }
        }
